...
>(
    ...
    for cluster_label in 0..*num_clusters {
        ...
        let table_for_shard_id_to_node_id: Vec<VectorIndex> = pickup_target_nodes(&cluster_label, cluster_labels);
        ...
        let (indexed_shard, start_shard_id): (Vec<(Point, Vec<u32>)>, u32) =
            vectune::Builder::default().set_seed(seed).progress(ProgressBar::new(1000)).build(shard_points);
...
fn pickup_target_nodes(cluster_label: &ClusterLabel, cluster_labels: &[(ClusterLabel, ClusterLabel)]) -> Vec<VectorIndex> {
    cluster_labels
            .iter()
            .enumerate()
